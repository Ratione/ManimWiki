...
class PlotFunctionGraph(Scene):
    ...
    def numpy_sin_function(self):
        function = np.sin
        wave = FunctionGraph(function,x_min=-5,x_max=5)
        self.play(ShowCreation(wave))

    def numpy_cos_function(self):
        function = np.cos
        wave = FunctionGraph(function,x_min=-5,x_max=5,color=RED)
        self.play(ShowCreation(wave))
    ...
    def damped_sin_wave(self):
        ...
        wave = FunctionGraph(function,x_min=-1,x_max=5)
        self.play(ShowCreation(wave))

    def sigmoid_function(self):

        function = helpers.sigmoid
        wave = FunctionGraph(function,x_max=6,x_min=-6)
        self.play(ShowCreation(wave))

    def soft_plus(self):
        function = lambda t: np.log(1+np.exp(t))
        curve = FunctionGraph(function,x_min=-6,x_max=7)
...
class MoveAlongPathAnimation(Scene):
    def construct(self):
        wave = FunctionGraph(np.sin,x_min=-5,x_max=4)
